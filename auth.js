document.addEventListener('DOMContentLoaded', () => {
    ...
    function processAllData(leadData, dealsVendasData, dealsLocacaoData) {
        // CORREÇÃO: Processa TODOS os leads, sem excluir nada aqui. As exclusões acontecem nos filtros.
        allLeads = leadData.map(row => {
            const tipoNegociacao = row['Tipo de Negociação'];
            if (!row['Criado'] || !tipoNegociacao) return null; // Apenas validação mínima
        ...
        const mqlsLocacao = dealsLocacaoData.map(row => {
            if (!row[mqlDateColumnLocacao] || !row['Data e hora da criação do Lead']) return null;
            return { id: row['ID'], nome: row['Nome do negócio'], mqlDate: parseDate(row[mqlDateColumnLocacao]), leadDate: parseDate(row['Data e hora da criação do Lead']), fonte: row['Fonte'] || 'Não Informada', unidade: 'MQL Locação' };
        }).filter(Boolean);
    ...
    function populateDynamicFilters(leads) {
        ...
        // Mantém os motivos de descarte marcados por padrão
        ...
        // Deixa os tipos de negociação desmarcados por padrão, como pediddo
    ...
    // O restante do script continua o mesmo, pois a lógica de ler os filtros já estava correta.
