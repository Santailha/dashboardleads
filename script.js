document.addEventListener('DOMContentLoaded', () => {
    ...
    const tiposNegociacaoExcluidosPadrao = ['não comercial'];
    ...
    Promise.all([ loadFile('leads.csv'), loadFile('deals_vendas.csv'), loadFile('deals_locacao.csv') ])
        .then(([leadData, dealsVendasData, dealsLocacaoData]) => {
            ...
            populateDynamicFilters(leadData); // Popula os novos filtros
            applyFilters(); // Aplica o filtro inicial
        }).catch(err => { console.error("Erro ao carregar os arquivos CSV:", err); tablesContainer.innerHTML = `<p style="color: red; text-align: center;">Erro ao carregar arquivos. Verifique os nomes dos arquivos.</p>`; });
    
    // === PROCESSAMENTO DE DADOS ===
    function processAllData(leadData, dealsVendasData, dealsLocacaoData) {
        // Agora, processamos TODOS os leads, sem excluir nada aqui.
    ...
    function populateDynamicFilters(leadData) {
        const motivos = [...new Set(leadData.map(lead => lead['Motivo de Descarte']).filter(Boolean))].sort();
        const tipos = [...new Set(leadData.map(lead => lead['Tipo de Negociação']).filter(Boolean))].sort();

        motivosDescarteContainer.innerHTML = motivos.map(motivo => `
            <label>
                <input type="checkbox" class="motivo-descarte-check" value="${motivo}" ${motivosDescartePadrao.includes(motivo) ? 'checked' : ''}>
                ${motivo}
            </label>
        `).join('');

        tiposNegociacaoContainer.innerHTML = tipos.map(tipo => `
            <label>
                <input type="checkbox" class="tipo-negociacao-check" value="${tipo}" ${tiposNegociacaoExcluidosPadrao.includes(tipo.toLowerCase()) ? 'checked' : ''}>
                ${tipo}
            </label>
        `).join('');
    }
    ...
    function applyFilters() {
        ...
        // Coleta os valores dos novos checkboxes de exclusão
    ...
    // O restante do script (populateFilter, updateDashboard, KPIs, tabelas, gráficos, modal) continua o mesmo
