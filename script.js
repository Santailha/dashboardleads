document.addEventListener('DOMContentLoaded', () => {
    ...
    Promise.all([ loadFile('leads.csv'), loadFile('deals_vendas.csv'), loadFile('deals_locacao.csv') ]).then(([leadData, dealsVendasData, dealsLocacaoData]) => { processAllData(leadData, dealsVendasData, dealsLocacaoData); applyFilters(); }).catch(err => { console.error("Erro ao carregar os arquivos CSV:", err); tablesContainer.innerHTML = `<p style="color: red; text-align: center;">Erro ao carregar arquivos. Verifique os nomes dos arquivos.</p>`; });
    function processAllData(leadData, dealsVendasData, dealsLocacaoData) {
        allLeads = leadData.map(row => { const tipoNegociacao = row['Tipo de Negociação']; if (!row['Criado'] || !tipoNegociacao || motivosDescarteLead.includes(row['Motivo de Descarte']) || tiposNegociacaoExcluidos.includes(tipoNegociacao.trim().toLowerCase())) return null; let unidade = (tipoNegociacao === 'Compradores') ? (bairrosCampeche.includes(row['Bairro Principal']) ? 'Vendas - Campeche' : 'Vendas - Centro') : tipoNegociacao; return { id: row['ID'], nome: row['Nome do Lead'], leadDate: parseDate(row['Criado']), fonte: row['Fonte'] || 'Não Informada', unidade: unidade, quantidade: 1 }; }).filter(Boolean);
    ...
    function showDrillModal(title, items, type = 'lead') {
        ...
        const nameHeader = type === 'lead' ? 'Nome do Lead' : 'Nome do Negócio';
        let tableHTML = `<table><thead><tr><th>ID</th><th>${nameHeader}</th></tr></thead><tbody>`;
        if (items.length > 0) {
            items.forEach(item => {
    ...
    function createSummaryTables(leads, mqls) {
        tablesContainer.innerHTML = ''; const unidades = [...new Set(leads.map(item => item.unidade))].sort();
        unidades.forEach(unidade => {
            const wrapper = document.createElement('div'); wrapper.className = 'table-wrapper';
            const title = document.createElement('h3'); title.textContent = unidade;
            ...
            fontes.forEach(fonte => {
                ...
                tableHTML += `<tr><td>${fonte}</td><td>${nLeads}</td><td class="mql-cell" data-fonte="${fonte}" data-unidade="${unidade}">${nMqls}</td><td>${conv}</td></tr>`;
            });
            const totalLeadsUnidade = leadsDaUnidade.length; const totalMqlsUnidade = mqlsDaUnidade.length; const convTotal = totalLeadsUnidade > 0 ? ((totalMqlsUnidade / totalLeadsUnidade) * 100).toFixed(0) + '%' : '0%';
            tableHTML += `</tbody><tfoot><tr><td>Total geral</td><td>${totalLeadsUnidade}</td><td class="mql-cell" data-fonte="all" data-unidade="${unidade}">${totalMqlsUnidade}</td><td>${convTotal}</td></tr></tfoot></table>`;
            wrapper.appendChild(title); wrapper.innerHTML += tableHTML; tablesContainer.appendChild(wrapper);
        });
    }
