document.addEventListener('DOMContentLoaded', () => {
    ...
    function createSummaryTables(leads, mqls) {
        ...
        unidades.forEach(unidade => {
            ...
            const convTotal = totalLeadsUnidade > 0 ? ((totalMqlsUnidade / totalMqlsUnidade) * 100).toFixed(0) + '%' : '0%';
    ...
    function showDrillModal(title, items) {
        ...
        let tableHTML = '<table><thead><tr><th>ID</th><th>Nome</th></tr></thead><tbody>';
        if (items.length > 0) {
            items.forEach(item => {
    ...
    function createLeadsPorFonteChart(data) {
        ...
        charts.leadsPorFonte = new Chart(ctx, { type: 'bar', data: { labels: sortedData.map(item => item[0]), datasets: [{ label: 'Total de Leads', data: sortedData.map(item => item[1]), backgroundColor: '#3d357e99', }] }, options: { scales: { x: { ticks: { precision: 0 } } }, indexAxis: 'y', plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'end', formatter: (value) => value > 0 ? value : '', } }, onClick: (event, elements) => { if (elements.length === 0) return; const index = elements[0].index; const fonteClicada = sortedData[index][0]; const leadsFiltrados = currentFilteredLeads.filter(lead => lead.fonte === fonteClicada); showDrillModal(`Leads da Fonte: ${fonteClicada}`, leadsFiltrados); } } });
    }
    ...
    function createLeadsPorUnidadeChart(data) {
        ...
        charts.leadsPorUnidade = new Chart(ctx, { type: 'pie', data: { labels: labels, datasets: [{ label: 'Total de Leads', data: dataValues, backgroundColor: ['#3d357e', '#edae0f', '#7c75b8', '#f2c75a', '#cccccc'], }] }, options: { plugins: { datalabels: { formatter: (value, ctx) => { let sum = ctx.chart.data.datasets[0].data.reduce((a, b) => a + b, 0); let percentage = (value * 100 / sum).toFixed(1) + "%"; return percentage; }, color: '#fff', font: { weight: 'bold', size: 14 } } }, onClick: (event, elements) => { if (elements.length === 0) return; const index = elements[0].index; const unidadeClicada = labels[index]; const leadsFiltrados = currentFilteredLeads.filter(lead => lead.unidade === unidadeClicada); showDrillModal(`Leads da Unidade: ${unidadeClicada}`, leadsFiltrados); } } });
    }
});
